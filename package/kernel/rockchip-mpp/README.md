...
### 错误：linux/wakelock.h 不存在

如果遇到 wakelock.h 头文件不存在的错误，这是因为：

- **原因**：`linux/wakelock.h` 在 Linux 内核 4.14+ 版本中已被移除，被 pm_wakeup 框架替代
- **修复**：代码已添加条件编译，在 Linux 6.12 内核中会跳过 wakelock.h 的包含
- **验证**：检查修复是否生效
  ```bash
  # 查看条件编译
  grep -A3 "wakelock.h" package/kernel/rockchip-mpp/src/rga/rga_drv.c
  ```
